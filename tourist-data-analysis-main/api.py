...
# 관광지별 반경 제한 (미터 단위)
PLACE_DISTANCE_LIMITS = {
    '경복궁': 200,      # 경복궁은 200m 반경
    '북촌한옥마을': 500,  # 북촌한옥마을은 500m 반경
    '청계천': 700,      # 청계천은 700m 반경
    # 다른 관광지들은 기본값 100m 사용
}
...
async def predict_location(
    ...
    if distance_m > PLACE_DISTANCE_LIMITS.get(target_place, DEFAULT_DISTANCE_LIMIT):
        return PredictionResponse(
            ...
            message=(
                ...
                f"{int(distance_m)}m 떨어져 있습니다. (최대 허용 거리: {PLACE_DISTANCE_LIMITS.get(target_place, DEFAULT_DISTANCE_LIMIT)}m)"
            )
        )
