...
app.mount("/test_images", StaticFiles(directory="test_images"), name="test_images") # [추가]
...
# --- [수정] TestPredictionRequest에 image_filename 추가 ---
...
async def _process_prediction(
    ...
    """
    이미지 경로를 받아 예측을 수행하고 결과를 반환하는 공통 함수
    """
...
# --- [수정] predict_with_test_image 로직 변경 ---
...
async def predict_with_test_image(request: TestPredictionRequest):
    try:
        ...
        # image_filename이 제공되면 해당 파일을 사용, 아니면 랜덤 선택 (기존 로직 유지)
...
async def get_test_images_for_place(place_name: str):
    ...
    try:
        ...
        # 클라이언트에서 서버 URL을 붙여 사용할 수 있도록 상대 경로로 반환
